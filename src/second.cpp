...
public:
    PointCloudProcessor() : Node("pointcloud_processor"){
        ...
        goals.push_back(std::make_pair(199, 90));   // in front of the seats
        goals.push_back(std::make_pair(199, 119));  // in front of the table from distance
        goals.push_back(std::make_pair(124, 119));  // in front of the table
        goals.push_back(std::make_pair(119, 90));   // behind of the setas
        goals.push_back(std::make_pair(119, 112));  // left of the table
        goals.push_back(std::make_pair(100, 100));  // end point
    }
    ...
    void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        if ((goals.size() == 5 || goals.size() == 2) && !detection_state){
            return;
        }
        ...
        if (align_grids && !goals.empty()){
            ...
            goal = {std::min(199, goals.front().first - bestMatchLoc.y), goals.front().second - bestMatchLoc.x}; // Example goal
            unchanged_goal = {std::min(199, goals.front().first), goals.front().second}; // Example goal
        }
        ...
        if ((goals.size() == 6 || goals.size() == 2 || goals.size() == 3) && task_audio_feedback) {
            task_audio_feedback = false;
            system("espeak \"Walk Left\"");
            RCLCPP_INFO(this->get_logger(), "Walk Left");
        } else if (goals.size() == 5 && task_audio_feedback) {
            task_audio_feedback = false;
            system("espeak \"Walk Right\"");
            RCLCPP_INFO(this->get_logger(), "Walk Right");
        } else if (goals.size() == 4 || goals.size() == 1) {
